...
def test_authors(tmp_path):
    ...
    id_bowie = db.add_author_to_authors_table("David Bowie", "David Bowie")
    assert id_bowie == 1

    id_queen = db.add_author_to_authors_table("queen", "queen")
    assert id_queen == 2
...
def test_series(tmp_path):
    ...
    id_silo = db.add_serie_to_series_table("Silo", "Silo")
    assert id_silo == 1

    id_got = db.add_serie_to_series_table("Game of Thrones", "Game of Thrones")
    assert id_got == 2
